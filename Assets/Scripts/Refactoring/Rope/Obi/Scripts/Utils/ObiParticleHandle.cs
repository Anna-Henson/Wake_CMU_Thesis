	...
	private void Actor_solver_OnFrameBegin (object sender, System.EventArgs e)
	{
		if (actor.InSolver){
			...
			for (int i = 0; i < handledParticleIndices.Count; ++i){
                ...
                Oni.GetParticlePositions(actor.Solver.OniSolver, movePos, 1, solverParticleIndex);
                for(int j = 0; j<pos.Length; ++j)
                    {
                        pos[j] = Vector3.Lerp(pos[j], movePos[j], lerp);
                        pos[j] += (Vector4)fieldForce;
                    }
