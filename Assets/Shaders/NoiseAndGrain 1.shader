	...
	SubShader {
		...
		Pass {  
            ...
            float cnoise( float3 pos){
            	...
            	return tex2D(_NoiseTex,pos.xy);

            }
            float vcnoise( float3 pos){

            	return tex2Dlod(_NoiseTex,float4(pos.xy,0,0));
			...
			v2f vert (appdata_t v){
				...
				float noise = _Wind * vcnoise(v.normal + _Time.y);
